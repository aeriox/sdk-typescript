...
import { AerioxDefaultError } from "../models/errors/aeriox-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AerioxDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AerioxDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AerioxDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AerioxDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
